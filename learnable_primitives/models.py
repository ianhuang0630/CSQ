...
class TulsianiNetwork(nn.Module):
    def __init__(self, network_params):
        ...
        input_channels = int(n_filters / 2)
        n_filters = 100
        for i in range(2):
...
def train_on_batch_pair(
    ...
    optimizer.zero_grad()
    # TODO: changes needed to the dataloader

    y_hat_from = model(X_from)
    y_hat_to = model(X_to)
    ...
    return (
        loss.item(),
        [x.data if hasattr(x, "data") else x for x in y_hat_from] + \
        [x.data if hasattr(x, "data") else x for x in y_hat_to] ,
        debug_stats
    )
