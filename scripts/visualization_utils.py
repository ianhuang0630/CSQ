...
def _from_primitive_parms_to_mesh(primitive_params):
    ...
    # XXX until .convex_hull is fixed
    m = trimesh.Trimesh(vertices=V.T)
    m = trimesh.convex.convex_hull(m, 'QbB Pp Qt')
...
def save_prediction_as_ply(primitive_files, filepath):
    ...
    face2prim_path = os.path.join(os.path.dirname(filepath), 'face2primitiveNumber.txt')
    face2prim = []
    ...
    for p in primitive_files:
        ...
        primitive_number = int(os.path.basename(p)[len("primitive_"):].split(".")[0])

        num_faces = _m.faces.shape[0]
        face2prim.extend([primitive_number] * num_faces)

        m = trimesh.util.concatenate(_m, m)

    m.export(filepath, file_type="ply")

    face2prim = face2prim[::-1]
    with open(face2prim_path, "w") as f:
        f.writelines("\n".join([str(el) for el in face2prim])+"\n")
    # saving face2prim
