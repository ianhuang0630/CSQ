...
from learnable_primitives.models import NetworkParameters, train_on_batch_w_parts, \
    optimizer_factory
...
RANDINDEX_SAMPLES_DIR = '/orion/u/ianhuang/superquadric_parsing/parts_randindex_samples'
        ...
        for b, sample in zip(range(args.steps_per_epoch), yield_infinite(train_bp)):
            ...
            for idx_batchwide, tag in enumerate(tags):
                ...
                target_part_samples_path = os.path.exists(
                    os.path.join(RANDINDEX_SAMPLES_DIR,
                                 '{}.pkl'.format(tag)))
                ...
                if os.path.exists(target_part_samples_path):
                    ...
                    curr_P = torch.sparse_coo_tensor(nonzero_indices, val, (N, N))
